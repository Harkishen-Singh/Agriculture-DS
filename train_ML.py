...
class Training_ML():
    ...
    def scannerHeadingNames(self):
        ...
        for heads in range(1, self.numberOfColumns+1):
            if self.ws.cell(row=1, column=heads).value == 'Block_Name' or self.ws.cell(row=1, column=heads).value == 'Block Name': # 0
                self.headingIndex[0]=heads
                self.correctInputFormat[0] = True
                print('came here 0')
            if self.ws.cell(row=1, column=heads).value == 'Village_Name' or self.ws.cell(row=1, column=heads).value == 'Village Name': # 1
                self.headingIndex[1]= heads
                self.correctInputFormat[1] = True;print('came here 1')
            if self.ws.cell(row=1, column=heads).value == 'Operational Size of the holding of farmer': # 2
                self.headingIndex[2] = heads
                self.correctInputFormat[2] = True;print('came here 2')
            if self.ws.cell(row=1, column=heads).value == 'Total area under crop(hectare) in respect of cultivator': # 3 Total area under crop(hectare) in respect of cultivator
                self.headingIndex[3] = heads
                self.correctInputFormat[3] = True;print('came here 3')

            if self.ws.cell(row=1, column=heads).value == 'System of Cultivation': # 4 System of Cultivation
                self.headingIndex[4] = heads
                self.correctInputFormat[4] = True;print('came here 4')

            if self.ws.cell(row=1, column=heads).value == 'Crop Variety Type': # 5
                self.headingIndex[5] = heads
                self.correctInputFormat[5] = True;print('came here 5')
            if self.ws.cell(row=1, column=heads).value == 'Variety Name':# 6
                self.headingIndex[6] = heads
                self.correctInputFormat[6] = True;print('came here 6')
            if self.ws.cell(row=1, column=heads).value == 'Sources of Seed':# 7
                self.headingIndex[7] = heads
                self.correctInputFormat[7] = True;print('came here 7')
            if self.ws.cell(row=1, column=heads).value == 'Seed Used per Hectare':# 8
                self.headingIndex[8] = heads
                self.correctInputFormat[8] = True;print('came here 8')
            if self.ws.cell(row=1, column=heads).value == 'Quantity of Manure/FYM used in(per hectare)':# 9
                self.headingIndex[9] = heads
                self.correctInputFormat[9] = True;print('came here 9')
            if self.ws.cell(row=1, column=heads).value == 'Quantity of Chemical Fertilizer (In hectare)':# 10
                self.headingIndex[10] = heads
                self.correctInputFormat[10] = True;print('came here 10')
            if self.ws.cell(row=1, column=heads).value == 'Time of showing or Transplanting': # 11
                self.headingIndex[11] = heads
                self.correctInputFormat[11] = True;print('came here 11')
            if self.ws.cell(row=1, column=heads).value == 'Date of likely harvest':# 12
                self.headingIndex[12] = heads
                self.correctInputFormat[12] = True;print('came here 12')
            if self.ws.cell(row=1, column=heads).value == 'Green Weight of the produce obtained in CCE' \
                    or self.ws.cell(row=1, column=heads).value == "Green Weight of the produce obtained in CCE;s in Kgs upto 5 gm":# 13
                self.headingIndex[13] = heads
                self.correctInputFormat[13] = True;print('came here 13')
            if self.ws.cell(row=1, column=heads).value == "Dry weight of the produce in CCE's" or \
                    self.ws.cell(row=1, column=heads).value== "Dry weight of the produce in CCE's(in Kgs. Upto 5 gm)" :# 14
                self.headingIndex[14] = heads
                self.correctInputFormat[14] = True;print('came here 14')
        ...
        ## ***** making all varieties to lowercase *********
        '''

        for i in range(2, self.numberOfRows+1):
            if type(self.ws.cell(row=i, column=self.headingIndex[6]).value) == str:
                self.ws.cell(row=i, column=self.headingIndex[6], value = self.ws.cell(row=i,column=self.headingIndex[6]).value.lower())
        '''
    def assignment(self): # to assign the values to the arrays declared above
        for i in range(2, self.numberOfRows+1):
            #if type(self.headingIndex[16].value) != None :
                print('line count :' +str(i))
